/*************************************************************************
	> File Name: SIPUtil.h
	> Author: cooperz
	> Mail: zbzcsn@qq.com
	> Created Time: Mon 13 Jul 2015 04:27:10 PM CST
 ************************************************************************/

#ifndef _SIPUTIL_H
#define _SIPUTIL_H

#define DEBUG 1
//#define GTEST 1
#define SIP_OUT 
#define SIP_IN 

#define LOCAL_DEV_NAME ("51012400002020000001") //202媒体服务器
#define SIP_SERVER_NAME ("51012400002000000001") //SIP服务器
#define LOCAL_DEV_PASSWD_STR ("12345678") 
#define REMOTE_DEV_NAME ("51012400001310000001") //1号摄像机
#define SENDER_VEDIO_SERIAL_NUM_STR ("1")
#define RECVER_VEDIO_SERIAL_NUM_STR ("1")

#define UAC_IP ("127.0.0.1")
#define UAC_LISTEN_PORT_STR ("5059")
#define UAC_LISTEN_PORT_INT (5059)

#define UAS_IP ("127.0.0.1")
#define UAS_LISTEN_PORT_STR ("5060")
#define UAS_LISTEN_PORT_INT (5060)

#endif
