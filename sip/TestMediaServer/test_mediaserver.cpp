...
{
    SIPMediaServer media_server(LOCAL_MED_NAME, UA_ME_IP, UA_ME_PORT_STR,
            LOCAL_DEV_PASSWD_STR);
    /*初始化，主要是初始化SIP解析器*/
...
int test_media_invite(int udpfd, SIPMediaServer *media_server)
{
    ...
    if(rt != SIP_CONTINUE)
    {
        cout<<"fiste invite recved, but build new register failed "<<endl;
        return -1; 
    }
    int wnum = sendto( udpfd, send_msg, (send_len), 0, (struct sockaddr *)&seraddr, 
            sizeof(seraddr));
    ...
    if(rt != SIP_CONTINUE)
    {
        cout<<"first invite ack recved, but build new register failed "<<endl;
        return -1; 
    }
    memset(rbuf,0,500);
    rnetnum = read( udpfd, rbuf , 500);
    ...
    if(rt != SIP_CONTINUE)
    {
        cout<<"second invite recved, but mediserver play  failed "<<endl;
        return -1; 
    }
    wnum = sendto( udpfd, send_msg, (send_len), 0, (struct sockaddr *)&seraddr, 
            sizeof(seraddr));
    if( wnum <= 0)
    {
        fprintf(stderr, "socket UDP: %s \n", strerror(errno));
        cout<<"invite recved, but mediserver play failed "<<endl;
        return -1;
    }
    memset(rbuf,0,500);
    rnetnum = read( udpfd, rbuf , 500);
    ...
    {
        cout<<"second invite ack recved, but mediserver play  failed:%d"<<rt<<endl;
        return -1;
    }
    return 1;
}
